...
class Fileband(object):
    ...
    def vbm(self):
        if self.is_metal():
            ...
            if len(self.__bands) == 2:
                res = max(res,max(list(v for v in self.__bands[1].flat if v < self.efermi)))
            return res
    ...
    def cbm(self):
        if self.is_metal():
            ...
            if len(self.__bands) == 2:
                res = min(res,min(list(v for v in self.__bands[1].flat if v > self.efermi)))
            return res
    ...
    '''
    def is_direct_gap(self):
        if self.is_metal():
            return None
        else:
            pos = []
            for x in range(self.no_bands):
                for y in range(self.band_length):
                    if self.__bands[x][y] == self.vbm():
                        pos.append((x,y))
            for x in range(self.no_bands):
                for y in range(self.band_length):
                    if self.__bands[x][y] == self.cbm():
                        if (x,y) in pos:
                            return True
            return False
    '''
    ...
    def export_to_csv(self,filename):
    	with open(filename,'w') as f:
    		for band in self.__bands:
    			for x, band_point in zip(self.x, band):
    				f.write('\"{:8.4f}\",\"{:16.10f}\"\n'.format(x,band_point))
    			f.write('\n')
    '''
            ...
            if len(self.__bands) == 2:
                for band in self.__bands[1]:
                    ...
                    for x,y in zip(self.x,band):
                        f.write('[{:8.4f},{:16.8f}],\n'.format(x,y))
                    f.write("],\ncolor:'#000000',},\n")
            f.write('];\nvar xmin = {:8.4f}, xmax = {:8.4f}, ymin = {:16.10f}, ymax = {:16.10f};'.format(self.xmin,self.xmax,self.bmin,self.bmax))
    ...
    def plot(self, foldname, funct, eps = True, svg = False):
        ...
        if svg:
            plt.savefig('{:s}/{:s}band.svg'.format(foldname, funct))
        plt.close()
