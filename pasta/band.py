...
class Fileband(object):
    ...
    def plot(self, foldname, funct, erela=True, emin = None, emax = None, eps=True, png=False):
        plt.figure()
    ...
    def fermi_dif(self):
        ...
        num = 0
        for i in range(self.no_bands):
            if np.any(self.__bands[0][i, :] > self.efermi):
                break
            dif += self.__bands[0][i][0]
            num +=1
        if len(self.__bands)==2:
            for i in range(self.no_bands):
                if np.any(self.__bands[1][i, :] > self.efermi):
                    break
                dif += self.__bands[1][i][0]
                num +=1
        if num==0:
            return 0
        else:
